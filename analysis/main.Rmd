...
dmd_ranks <-
  isoforms_data %>%
  mutate(across(c(starts_with("HSC"), starts_with("MPP")), ~ min_rank(desc(.x)))) %>%
...
.n_isoforms <- as.list(rep(dim(isoforms_data)[1], 16))
names(.n_isoforms) <- names(dmd)

dmd_ranks %>%
  bind_rows(c(.n_isoforms, Name = "n")) %>%
  bind_rows(c(as.list(dmd / hprt * 100), Name = "Dmd / Hprt [%]")) %>%
  bind_rows(c(as.list(dmd / gapdh * 100), Name = "Dmd / Gapdh [%]")) %>%
  select(-tracking_id, -gene_id, -gene_short_name) %>%
  relocate(Name) %>%

  mutate(HSC = round(rowMeans(select(., starts_with("HSC"))), 2)) %>%
  mutate(MPP1 = round(rowMeans(select(., starts_with("MPP1"))), 2)) %>%
  mutate(MPP2 = round(rowMeans(select(., starts_with("MPP2"))), 2)) %>%
  mutate(MPP3 = round(rowMeans(select(., starts_with("MPP3"))), 2)) %>%
  mutate(MPP4 = round(rowMeans(select(., starts_with("MPP4"))), 2)) %>%

  mutate(ALL = round(rowMeans(select(., contains("_"))), 3)) %>%
  select(-contains("_")) %>%
